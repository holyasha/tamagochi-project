package com.example.tamagochi.grpcanalytics.config;

import com.example.tamagochi.grpcanalytics.service.TamagochiAnalyticsServiceImpl;
    ...
    public void start() {
        try {
            server = ServerBuilder.forPort(grpcPort)
                    .addService(new TamagochiAnalyticsServiceImpl())
                    .build()
                    .start();

            running = true;
            log.info("gRPC-сервер запущен на порту {}", grpcPort);
            log.info("Сервис: TamagochiAnalytics.AnalyzeTamagochi()");
